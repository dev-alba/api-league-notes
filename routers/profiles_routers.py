...
@profile_router.get(
    "/{nickname}/{tagline}", status_code=200, response_model=ProfileResponse
)
def get_profile_by_nickname_router(
    nickname: str,
    tagline: str,
...
def get_profile_list_by_user_id_router(
...
def create_profile_router(
    ...
    return profiles_services.create_profile_service(
        db, nickname, tagline, current_user.id
    )
...
def update_profile_router(
    ...
    return profiles_services.update_profile_service(
        db, current_user.id, nickname, tagline, data.new_nickname, data.new_tagline
    )
...
def delete_profile(
    ...
    return profiles_services.delete_profile_service(
        db, current_user.id, nickname, tagline, data.password
    )
