...
def create_user_router(data: UserCreate, db=Depends(get_db)):
    return users_services.create_user_service(
        db, data.nickname, data.email, data.password
    )


@user_router.patch("/", status_code=200, response_model=UserResponse)
def update_user_password_router(
    data: UserUpdatePassword,
    current_user: int = Depends(get_current_user),
    db=Depends(get_db),
):
    return users_services.update_user_service(
        db, current_user.id, data.password, data.new_password
    )


@user_router.post("/delete-confirmation", status_code=204)
def delete_user_router(
    data: UserDelete, current_user=Depends(get_current_user), db=Depends(get_db)
):
