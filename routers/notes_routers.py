...
def create_note_router(nickname: str, tagline: str, content: str, current_user=Depends(get_current_user), db=Depends(get_db)):
    return notes_services.create_note_service(db, current_user.id, nickname, tagline, content)
    
@note_router.patch('/update', status_code=200, response_model=NoteResponse)
def update_note_router(nickname: str, tagline: str, note_id: int, new_content: str, current_user=Depends(get_current_user), db=Depends(get_db)):
    return notes_services.update_note_service(db, current_user.id, nickname, tagline, note_id, new_content)
