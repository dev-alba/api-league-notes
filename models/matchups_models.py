...
class Matchup(Base):
    ...
    player_champion: Mapped["Champion"] = relationship(
        "Champion", foreign_keys=[player_champion_id]
    )
    enemy_champion: Mapped["Champion"] = relationship(
        "Champion", foreign_keys=[enemy_champion_id]
    )

    __table_args__ = (
        UniqueConstraint(
            "player_champion_id", "enemy_champion_id", name="_UC_champ_id_enemy_id"
        ),
    )
