def test_create_profile(client, token_headers):
    data = {"nickname": "User", "tagline": "TEST"}
...
def test_update_profile(client, token_headers):
    ...
    new_data = {"new_nickname": "John", "new_tagline": "Doe"}
    response = client.patch(
        "/profiles/", params=data, json=new_data, headers=token_headers
    )
...
def test_delete_profile(client, token_headers):
    ...
    profile_data = {"nickname": "User", "tagline": "TEST"}
    password = {"password": "password"}
    nickname = profile_data["nickname"]
    tagline = profile_data["tagline"]

    response = client.post(
        f"/profiles/{nickname}/{tagline}/delete-confirmation",
        params=profile_data,
        json=password,
