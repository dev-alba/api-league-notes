def test_create_note(client, token_headers, set_champions):
    payload = {"player_champion": "LeeSin", "enemy_champion": "Khazix"}
    client.post("/matchups/", params=payload, headers=token_headers)

    profile = {"nickname": "User", "tagline": "TEST"}
...
def test_get_notes_by_matchup(client, token_headers, set_champions):
    ...
    response = client.get(
        "/notes/get-by-matchup", params=payload, headers=token_headers
    )
    assert response.status_code == 200
...
def test_update_note(client, token_headers, set_champions):
    ...
    new_note = {"content": "New content"}
    response = client.patch(
        f"/notes/update/{note_id}",
        params=old_note,
        json=new_note,
...
def test_delete_note(client, token_headers, set_champions):
    ...
    response = client.delete(
        f"/notes/delete/{note_id}", params=note, headers=token_headers
    )
