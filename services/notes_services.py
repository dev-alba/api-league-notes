...
def get_notes_by_profile_service(db, user_id, nickname, tagline):
    profile = profiles_repository.auth_profile_user_id_repo(
        db, user_id, nickname, tagline
    )
    if not profile:
...
def get_notes_by_matchup_service(
    db, user_id, nickname, tagline, player_champion, enemy_champion
):
    profile = profiles_repository.auth_profile_user_id_repo(
        db, user_id, nickname, tagline
    )
...
def create_note_service(
    db, user_id, nickname, tagline, player_champion_name, enemy_champion_name, content
):
    profile = profiles_services.auth_profile_service(db, user_id, nickname, tagline)
    matchup = matchups_service.get_matchup_by_names(
        db, player_champion_name, enemy_champion_name
    )
    if not matchup:
        raise MatchupNotFound
    note = Note(matchup_id=matchup.id, profile_id=profile.id, content=content)
